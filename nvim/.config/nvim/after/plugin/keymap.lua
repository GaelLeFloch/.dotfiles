...
nnoremap('<leader>r', ':w | silent !tmux send-keys -t bottom-right "fc -e -" Enter<CR>')
...
nnoremap("<leader>m", ':Telescope file_browser initial_mode=normal path=%:p:h select_buffer=true<CR>', silent)
...
nnoremap('<leader>gv', ":lua require'telescope.builtin'.git_bcommits{} <CR>", {noremap = true, silent=true})
